  ...
  return (
    <Link
      ...
      onMouseOut={(event) => {
        ...
        event.currentTarget.style.boxShadow = '0 4px 14px rgba(15, 23, 42, 0.035)'
    ...
    >
      ...
      <div style={{ display: 'grid', gap: 8 }}>
        ...
        <div style={{ display: 'grid', gap: 5 }}>
          <div style={{ fontSize: 15, fontWeight: 600, letterSpacing: '-0.04em', color: 'var(--dark)' }}>{product.name}</div>
          <div style={{ fontSize: 12, lineHeight: 1.45, color: 'var(--mid)', minHeight: 34 }}>
            {product.description || `Configured ${product.brand} hardware for business teams and personal upgrades.`}
          </div>
        </div>

        <div style={{ display: 'flex', alignItems: 'end', justifyContent: 'space-between', gap: 12, paddingTop: 2 }}>
          <div style={{ display: 'grid', gap: 2 }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--light)' }}>
              From
            </div>
            <div style={{ fontSize: 18, fontWeight: 600, letterSpacing: '-0.04em', color: 'var(--dark)' }}>
