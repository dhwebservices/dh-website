...
const WHATSAPP_NUMBER = '447359587007' // Replace with real WhatsApp number
...
export default function WhatsAppButton() {
  ...
  useEffect(() => {
    ...
    // Pulse once after 6 seconds to draw attention
    const p = setTimeout(() => setPulsed(true), 6000)
    const p2 = setTimeout(() => setPulsed(false), 7500)
  ...
  return (
    <>
      <style>{`
        ...
        .wa-btn.pulsing {
          animation: wa-pulse 1s ease-out 3;
        }
      `}</style>
      ...
      {visible && (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className={`wa-btn${pulsed ? ' pulsing' : ''}`}
