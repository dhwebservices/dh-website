...
export default function WhatsAppButton() {
  ...
  useEffect(() => {
    const desktopOnly = window.matchMedia('(hover: hover) and (pointer: fine)').matches
    if (!desktopOnly) return undefined

    const delay = window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 0 : 8000
  ...
  return (
    <>
      <style>{`
        ...
        .wa-btn {
          animation: wa-in 0.4s cubic-bezier(0.16,1,0.3,1) both;
          box-shadow: 0 10px 30px rgba(37,211,102,0.18);
          transition: transform 0.2s cubic-bezier(0.16,1,0.3,1), box-shadow 0.2s;
        }
        .wa-btn:hover {
          transform: translateY(-1px) !important;
          box-shadow: 0 14px 32px rgba(37,211,102,0.24) !important;
        }
      `}</style>
      ...
      {visible && (
        ...
        >
          ...
          <span style={{ color: '#fff', fontSize: 14, fontWeight: 600, letterSpacing: '-0.01em' }}>WhatsApp us</span>
        </a>
      )}
    </>
  )
