/**
 * The Contact page as an app block.
 *
 * Moved here wholesale from src/pages/Contact.jsx rather than retyped. It
 * carries the enquiry form, the calculator hand-off, the booking widget and
 * two outbound emails - retyping 200 lines of that to make it "cleaner" is how
 * a working lead-capture form quietly stops working.
 *
 * Only the surrounding copy is parameterised. Field labels, validation and the
 * email templates stay in code on purpose: they are wired to what the emails
 * expect, so they are not safe to edit freely.
 */
...
export function ContactBlock({ eyebrow, heading, intro, emailEyebrow, emailHeading, successHeading, successBody, geoEyebrow, geoBody }) {
          ...
          <div id="brief" className="reveal section-narrow contact-brief-panel" style={{ marginTop:72 }}>
            <div className="glass-card" style={{ borderRadius:20, padding:'clamp(28px,4vw,40px)' }}>
              <div style={{ marginBottom:24 }}>
                <p className="eyebrow" style={{ marginBottom:12 }}>{emailEyebrow}</p>
                <h2 className="headline-md" style={{ marginBottom:12 }}>{emailHeading}</h2>
              ...
              {sent ? (
                <div style={{ padding:'32px 0' }}>
                  <div style={{ textAlign:'center', marginBottom:32 }}>
                    ...
                    <h3 style={{ fontSize:22, fontWeight:600, marginBottom:8, color:'var(--dark)' }}>{String(successHeading || '').replace('{firstName}', form.name.split(' ')[0])}</h3>
                    <p className="body-md" style={{ color:'var(--mid)', maxWidth:400, margin:'0 auto' }}>{successBody}</p>
                  </div>
