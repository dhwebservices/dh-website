/**
 * The pricing calculator as an app block.
 *
 * Moved here wholesale from src/pages/Calculator.jsx rather than retyped - it
 * is quoting logic that feeds a real enquiry, so a transcription slip would
 * quote the wrong price rather than look slightly wrong.
 *
 * Prices are editable, the arithmetic is not. The sum reads .cost off whatever
 * options the document supplies, so changing a price is safe.
 *
 * Known coupling, left as it was: the suggested package thresholds (449 / 999
 * / 1499) are hardcoded and do not follow the prices above them. Change the
 * package prices and the suggestion boundaries stay where they are.
 */
...
export function CalculatorBlock({
  ...
  // editing a price is safe, and editing the arithmetic is not on offer.
  const PAGE_OPTIONS = pageOptions?.length ? pageOptions : PAGE_OPTIONS_DEFAULT
  const FEATURES = features?.length ? features : FEATURES_DEFAULT
  const DESIGN_OPTIONS = designOptions?.length ? designOptions : DESIGN_OPTIONS_DEFAULT
  const HOSTING_OPTIONS = hostingOptions?.length ? hostingOptions : HOSTING_OPTIONS_DEFAULT
  const BASE_PRICE = Number(basePrice) || BASE_PRICE_DEFAULT
  ...
  const selectedFeatureLabels = [...selected]
  ...
  const contactParams = new URLSearchParams({
    source: 'calculator',
    package: suggest,
    total: String(totalBuild),
    monthly: String(monthlyCost),
    pages,
    design,
    hosting,
  })
  if (selectedFeatureLabels.length) {
    contactParams.set('features', selectedFeatureLabels.join(', '))
  }
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 320px', gap:40, alignItems:'start' }}>
            ...
            <div style={{ position:'sticky', top:'calc(var(--nav-h) + 24px)' }}>
              <div style={{ background:'var(--dark)', borderRadius:20, padding:'28px 24px', color:'#fff' }}>
                ...
                {/* CTA */}
                <Link to={contactHref} style={{ display:'block', textAlign:'center', padding:'14px', borderRadius:100, background:'#fff', color:'var(--dark)', textDecoration:'none', fontSize:14, fontWeight:700, letterSpacing:'0.01em', transition:'opacity 0.15s' }}
                  onClick={() => trackEvent('calculator_contact_click', { package: suggest, total: totalBuild })}
