...
async function writeRedirects() {
  const lines = [
    ...
    '',
    '# NOTE: apex -> www is deliberately NOT here. Pages _redirects matches on',
    '# path only, never on hostname, because both custom domains are served from',
    '# the same deployment -- a rule with a hostname in it is silently ignored.',
    '# Duplicate content is handled instead by the canonical link tag emitted',
    '# into every prerendered page, which points at the www URL. For an actual',
...
console.log(`sitemap: ${urlCount} urls, _redirects: ${redirectCount} redirects`)
